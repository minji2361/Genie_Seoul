...
export default function ProgramsSection() {
  ...
  return (
    <section id="programs" className="py-24 bg-[#F5F5F5]">
      <div className="container-genie">
        ...
        <div className="flex flex-col lg:flex-row lg:items-end lg:justify-between mb-10 gap-4">
          ...
          <Link
            href="/program"
            className="self-start lg:self-auto font-bold text-sm text-[#111] border-b-2 border-[#FFE600] pb-0.5 hover:border-[#111] transition-colors whitespace-nowrap"
          >
            전체 프로그램 보기 →
          </Link>
        </div>
        ...
        {/* Desktop: featured + sidebar list */}
        {active === "all" && (
          <div className="grid lg:grid-cols-[1fr_380px] gap-[2px] mb-[2px] bg-[#111]">

            {/* Featured large card */}
            <Link href={`/program/${featured.id}`} className="block">
              <div className="bg-[#FFE600] p-8 lg:p-10 flex flex-col justify-between min-h-[320px] h-full hover:bg-[#f5dc00] transition-colors cursor-pointer">
                <div>
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <span className={`text-xs font-bold px-2.5 py-1 ${STATUS_COLOR[featured.status]}`}>
                      {featured.status}
                    </span>
                  </div>
                  <h3 className="font-display text-4xl lg:text-5xl text-[#111] mb-2 leading-tight">
                    {featured.title}
                  </h3>
            ...
            {/* Sidebar: remaining programs */}
            <div className="flex flex-col gap-[2px]">
              {PROGRAMS.slice(1, 4).map((prog) => (
                <Link key={prog.id} href={`/program/${prog.id}`} className="flex-1 block">
        ...
        <div className="mt-8 flex justify-center">
          <Link href="/program" className="btn-outline px-10 py-4 text-base">
            전체 프로그램 보기 →
          </Link>
        </div>
      </div>
